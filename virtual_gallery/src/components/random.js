...
const renderResult = (result, resultsList) => {
	...
	addToPlaylistModal.className = 'waves-effect waves-light btn green modal-trigger';
...
const postPlaylist = (playlistObj) => {
	return fetch('http://localhost:3000/playlists', {
		...
		.then((res) => res.json())
		.then((playlistObj) => {
			currentUser.data.playlist = [ ...currentUser.data.playlist, playlistObj.data ];
...
const iteratePlaylists = (user) => user.data.playlist.forEach((playlist) => showPlaylist(playlist));
...
const showPlaylist = (playlist) => {
	...
	durationTotal.length
		? (durationSpanEl.innerText = 'Duration: ' + (durationTotal.reduce(reducer) / 60).toFixed(2))
...
const fetchTrackInfo = (track, trackBodyEl) => {
	return fetch(`https://deezerdevs-deezer.p.rapidapi.com/track/${track.deezer_track_id}`, {
		method: 'GET',
		headers: {
			'x-rapidapi-host': 'deezerdevs-deezer.p.rapidapi.com',
			'x-rapidapi-key': 'd0e26413acmsh8b0c66313f1ff6cp14b3c7jsn84dd8c7ba916'
...
const renderTrackInfo = (track, trackBodyEl) => {
	...
	trackBodyEl.append(trackAlbumImageEl, trackTitleEl, trackArtistEl, trackAlbumEl, trackAudioEl);
	addEffects();
};
