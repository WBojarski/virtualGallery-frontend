...
export default class HomePageCardsContainer extends Component {
	...
	componentDidMount() {
		...
		fetch(artPiecesURL).then((resp) => resp.json()).then((artPieces) =>
			this.setState({
				artPieces: artPieces
			})
		);
	...
	renderArtPieceCards = (artPieces) => {
		if (artPieces && artPieces.length > 0) {
			return artPieces.map((artPiece) => {
				...
				return <HomePageCard artPiece={artPiece} key={artPiece.id} />;
			});
		}
	};
	...
	render() {
		return (
			<div>
				<Container className="card-container">{this.selectRandomArtPieces(this.state.artPieces)}</Container>
			</div>
		);
	}
}
